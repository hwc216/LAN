...
uint8_t Ctrl_Mode = 0; // 控制模式 0-自动控制 1-手动控制
float Ctrl_Mode_Duty;
float ADC_Value;
...
int main(void)
{
  ...
  PWM_Set_Frequency(&htim16, TIM_CHANNEL_1, 100);
  PWM_Set_Frequency(&htim17, TIM_CHANNEL_1, 200);
...
void LCD_Proc(void)
{
  ...
  if (LCD_Disp_Changle == 0)
  {
    ...
    LCD_DisplayStringAtLineColumn(Line1, Col7, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "V:%.2fV  ", ADC_Value);
    LCD_DisplayStringAtLineColumn(Line3, Col5, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Mode:%s  ", Ctrl_Mode ? "MANU" : "AUTO");
    LCD_DisplayStringAtLineColumn(Line5, Col5, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "PA6:%d  %.2f    ", frq1, duty1);
    LCD_DisplayStringAtLineColumn(Line6, Col5, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "PA7:%d  %.2f    ", frq2, duty2);
...
void KEY_Proc(void)
{
  ...
  {
    Ctrl_Mode_Duty = ADC_Value / 3.3f * 100.0f;
    PWM_Set_Duty(&htim16, TIM_CHANNEL_1, Ctrl_Mode_Duty);
    PWM_Set_Duty(&htim17, TIM_CHANNEL_1, Ctrl_Mode_Duty);
    if (ADC_Value > 3.29f)
    {
      PWM_Set_Duty(&htim16, TIM_CHANNEL_1, 100);
