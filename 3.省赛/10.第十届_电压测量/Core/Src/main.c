...
float Max_Volt = 2.4f, Min_Volt = 1.2f;
int8_t Max_LED_Set = 1, Min_LED_Set = 2;
float Volt_Current;
uint8_t Volt_Level;

const char *Volt_Level_String[3] = {"Lower", "Nomal", "Upper"}; // 使用指针数组
...
int main(void)
{
  ...
  Volt_Current = Get_ADC_Average(&hadc2, 10); // 上电先读取一次ADC，防止数据跳变
...
void LED_Proc(void)
{
  ...
  if (Volt_Level == 2)
  {
    ucLED &= (1 << (Max_LED_Set - 1));
    ucLED ^= (1 << (Max_LED_Set - 1));
  }
  else if (Volt_Level == 0)
  {
    ucLED &= (1 << (Min_LED_Set - 1));
    ucLED ^= (1 << (Min_LED_Set - 1));
  }
  else
  {
...
void LCD_Proc(void)
{
  ...
  Volt_Current = Get_ADC_Average(&hadc2, 10);
  if (Volt_Current <= Min_Volt)
    Volt_Level = 0;
  else if (Volt_Current <= Max_Volt)
    Volt_Level = 1;
  else if (Volt_Current > Max_Volt)
    Volt_Level = 2;
  ...
  {
    LCD_SetBackColor(Black); // 初始背景颜色 防止受上一个对背景颜色的改变影响
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Volt: %.2fV  ", Volt_Current);
    LCD_DisplayStringAtLineColumn(Line4, Col4, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Status: %s    ", Volt_Level_String[Volt_Level]);
    LCD_DisplayStringAtLineColumn(Line6, Col4, LCD_String_Disp);
  }
  else
  {
    LCD_SetBackColor(Black); // 初始文本颜色 防止受上一个对文本颜色的改变影响
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Max Volt:%.1f    ", Max_Volt);
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Min Volt:%.1f    ", Min_Volt);
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Upper:LD%d       ", Max_LED_Set);
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "Lower:LD%d       ", Min_LED_Set);
