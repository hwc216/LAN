...
__IO uint32_t uwTick_Receive_Time_Count;
...
float ADC_Value;
uint8_t Nation_flag = 0;             // 0-直行S 1-左转L  2-右转R
uint8_t Left_ok_flag, Right_ok_flag; // 左转完成标志位 右转完成标志位
uint8_t Receive_flag;                // 5s接收标志位
uint8_t Once_flag;

float Distance;

const char *Nation_buff[3] = {"S", "L", "R"}; // 使用指针数组
...
void LCD_Proc(void)
{
  ...
  if (uwTick - uwTick_Time_Count > 5000 && Nation_flag != 0 && Once_flag == 0)
  {
    LCD_Disp_Change = 1; // 切换到偏航界面
    Nation_flag = 0;
    Once_flag = 1;
    LCD_Clear(Black);
    printf("WARN\r\n");
  ...
  if (uwTick - uwTick_Receive_Time_Count > 5000 && Receive_flag == 1)
  {
    Receive_flag = 0;
  }

  Distance = ADC_Value * 100.0f;
  if (ADC_Value > 3.0f)
    Distance = 300.0f;
  ...
  if (LCD_Disp_Change == 0)
  {
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "N:%s  ", Nation_buff[Nation_flag]); // s直行 R右转 L左转
    LCD_DisplayStringAtLineColumn(Line4, Col8, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "D:%.1f  ", Distance);
    LCD_DisplayStringAtLineColumn(Line5, Col8, LCD_String_Disp);
  }
  else
  {
    LCD_SetTextColor(White); // 初始文本颜色 防止受上一个对文本颜色的改变影响
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "WARN  ");
    LCD_DisplayStringAtLineColumn(Line5, Col9, LCD_String_Disp);
  }
}
/*****************************************按键控制函数***************************************************/
void KEY_Proc(void)
{
  ...
  if (single_key_flag[0] == 1) // 按键1 调头按键
  {
    // 在偏离导航界面下，按下B1 掉头成功  回到车辆行驶界面  方向改为S
    if (LCD_Disp_Change) // 偏离导航界面
    {
      LCD_Disp_Change = 0; // 切换为车辆行驶界面
      LCD_Clear(Black);
      Nation_flag = 0;
      printf("Success\r\n");
    }
  ...
  {
    if (LCD_Disp_Change == 0) // 车辆行驶界面
    {
      // 当串口发送导航方向为左转时， 5s内按下按键左转 完成后导航切换为直行s 否则界面切换到偏离导航界面
      // 若导航为直行s 按下按键切换到偏航界面
      if (Nation_flag == 1 && uwTick - uwTick_Time_Count < 5000)
      {
        Nation_flag = 0;
        Left_ok_flag = 1;
        printf("Success\r\n");
      }
      else if (Nation_flag == 0)
      {
        LCD_Disp_Change = 1; // 切换到偏航界面
        LCD_Clear(Black);
        printf("Warn\r\n");
  ...
  if (single_key_flag[2] == 1) // 按键3单击
  {
    if (LCD_Disp_Change == 0)
    {
      // 当串口发送导航方向为右转时， 5s内按下按键右转 完成后导航切换为直行s 否则界面切换到偏离导航界面
      // 若导航为直行s 按下按键切换到偏航界面
      if (Nation_flag == 2 && uwTick - uwTick_Time_Count < 5000)
      {
        Nation_flag = 0;
        Right_ok_flag = 1;
        printf("Success\r\n");
...
void USART_Proc(void)
{
  ...
  if (rx_pointer > 0 && Receive_flag == 0) // 5s内只能接受一次
  {

    if (rx_pointer == 1 && LCD_Disp_Change == 0)
    {
      if (rxdata[0] == 'L')
      {
        Nation_flag = 1;  // 左转
        Receive_flag = 1; // 接受标志位置1
        Once_flag = 0;
        uwTick_Time_Count = uwTick; // 开始计时
        uwTick_Receive_Time_Count = uwTick;
      }
    ...
    else if (rx_pointer == 1 && LCD_Disp_Change == 1)
      printf("WAIT\r\n");
    else
      printf("ERROR\r\n");
  }
