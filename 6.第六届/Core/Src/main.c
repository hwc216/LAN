...
uint8_t LCD_Line_Changle = 0; // 显示屏选择行 1-第一行 2-第二行 3-第三行 4-第四行 默认为0不选择 无高亮文本

uint8_t PA6_duty = 10, PA7_duty = 10;
uint16_t PA6_Frq = 100, PA7_Frq = 200;

uint8_t EEPROM_Save[4] = {1}; // EEPROM芯片中存取的数据
uint8_t EEPROM_Disp[4] = {1}; // EEPROM数据显示数组

uint8_t LED_2_statue = 0; // 报警灯开关变量
float K = 0.1;
...
void LED_Proc(void)
{
  ...
  if (LED_2_statue)
  {
    if (Get_ADC(&hadc2) > (3.3f * K))
    {
      ucLED ^= 0x01;
    }
    else if (Get_ADC(&hadc2) < (3.3f * K))
    {
      ucLED &= 0x00;
    }
  }
...
void LCD_Proc(void)
{
  if ((uwTick - uwTick_LCD_Speed_Ctrl) < 200)
    return;
  uwTick_LCD_Speed_Ctrl = uwTick;

  Sent_count = 0;

  I2C_24c02_Read(EEPROM_Disp, 0x00, 1); // 读取EEprom中的值 断电记忆
  K = EEPROM_Disp[0] / 10.0;
  ...
  if (LCD_Disp_Changle == 0)
  {
    ...
    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "    V1: %.2fV", Get_ADC(&hadc2));
    LCD_DisplayStringLine(Line3, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "    K : %0.1f", K);
    LCD_DisplayStringLine(Line4, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "    LED: %s    ", (LED_2_statue == 1) ? "ON" : "OFF");
    LCD_DisplayStringLine(Line5, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "    T: %02d-%02d-%02d  ", RTC_Time.Hours, RTC_Time.Minutes, RTC_Time.Seconds);
  ...
  else
  {
    ...
    LCD_DisplayStringAtLineColumn(Line1, Col8, LCD_String_Disp);

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "%02d", set_time[0]);
    ...
    LCD_DisplayStringAtLineColumn(Line4, Col7, LCD_String_Disp);

    LCD_SetTextColor(White);
    LCD_DisplayChar(Line4, Col9, '-');

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "%02d", set_time[1]);
    ...
    LCD_SetTextColor(White);
    LCD_DisplayChar(Line4, Col12, '-');

    memset(LCD_String_Disp, 0, sizeof(LCD_String_Disp));
    sprintf((char *)LCD_String_Disp, "%02d", set_time[2]);
    ...
    LCD_DisplayStringAtLineColumn(Line4, Col13, LCD_String_Disp);
  }
}
/*****************************************按键控制函数***************************************************/
void KEY_Proc(void)
{
  ...
  if (single_key_flag[0] == 1) /*******************************按键1切换报警灯状态************************/
  {
    if (LCD_Disp_Changle == 0)
    {
      LED_2_statue ^= 0x01;
  ...
  if (single_key_flag[1] == 1) /*******************************按键2单击刷新显示屏************************/
  {
  ...
  if (single_key_flag[2] == 1) /*******************************按键3单击选择高亮数据************************/
  {
    if (LCD_Disp_Changle)
    {
  ...
  if (single_key_flag[3] == 1) /*******************************按键4单击修改数据************************/
  {
...
void USART_Proc(void)
{
  ...
  if (rx_pointer > 0)
  {
    if (rx_pointer == 5)
    {
      if (rxdata[0] == 'k')
      {
        if (rxdata[1] == '0' && strchr(rxdata, '\n'))
        {
          K = (rxdata[3] - '0') / 10.0;
          EEPROM_Save[0] = rxdata[3] - '0';
          I2C_24c02_Write((uint8_t *)EEPROM_Save, 0x00, 1); // 将阈值写入EEPROM中
          HAL_Delay(10);
          printf("ok\r\n");
        }
...
void RTC_Proc(void)
{
  ...
  if (RTC_Time.Hours == set_time[0] && RTC_Time.Minutes == set_time[1] && RTC_Time.Seconds == set_time[2])
  {
    if (Sent_count == 0)
    {
      Sent_count = 1;
      printf("%.2f+%.1f+%02d%02d%02d\r\n", Get_ADC(&hadc2), K, RTC_Time.Hours, RTC_Time.Minutes, RTC_Time.Seconds);
    }
  }
